const {
 ...
 cmd({
   pattern: "promote",
desc: "Provides admin role to replied/quoted user or everyone (with promoteall)",
category: "group",
filename: __filename,
use: "<quote|reply|number|promoteall>"
}, async _0x324f8b => {
  try {
    ...
    if (_0x324f8b.command === "promoteall") {
      const groupMetadata = await _0x324f8b.bot.groupMetadata(_0x324f8b.chat);
      const participants = groupMetadata.participants;
      const nonAdmins = participants.filter(p => !p.admin);

      if (nonAdmins.length === 0) {
        return _0x324f8b.reply("*_All members are already admins!_*");
      }

      const nonAdminJids = nonAdmins.map(p => p.id);
      await _0x324f8b.bot.groupParticipantsUpdate(_0x324f8b.chat, nonAdminJids, "promote");

      return _0x324f8b.reply(`*_Promoted ${nonAdminJids.length} members to admin successfully ✅!_*`);
    }
    ...
    let _0x8f9e68 = _0x324f8b.mentionedJid[0]
      ...
      : false;
    if (!_0x8f9e68) {
      return await _0x324f8b.reply("*Uhh dear, reply/mention a User*");
    }

    await _0x324f8b.bot.groupParticipantsUpdate(_0x324f8b.chat, [_0x8f9e68], "promote");
    await _0x324f8b.send(
      "*_@" + _0x8f9e68.split("@")[0] + " promoted Successfully ✅!_*",
      { mentions: [_0x8f9e68] }
    );
 ...
 cmd({
   pattern: "demote",
desc: "Demotes replied/quoted user from group or everyone (with demoteall)",
category: "group",
filename: __filename,
use: "<quote|reply|number|demoteall>"
}, async _0x118677 => {
  try {
    ...
    // Vérification pour la commande demoteall
    if (_0x118677.command === "demoteall") {
      const groupMetadata = await _0x118677.bot.groupMetadata(_0x118677.chat);
      const participants = groupMetadata.participants;
      const admins = participants.filter(p => p.admin); // Sélectionne uniquement les admins

      if (admins.length === 0) {
        return _0x118677.reply("*_No admins left in the group!_*");
      }

      const adminJids = admins.map(p => p.id);
      await _0x118677.bot.groupParticipantsUpdate(_0x118677.chat, adminJids, "demote");

      return _0x118677.reply(`*_Demoted ${adminJids.length} admins successfully ✅!_*`);
    }

    // Commande normale demote
    ...
    try {
      ...
      await _0x118677.reply("*_User demoted successfully ✅!!_*");
    } catch (_0x5e7b02) {
